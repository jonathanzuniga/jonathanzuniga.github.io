...
(function($) {
    ...
    Plugin.prototype.setNotes = function(supers, notes) {
        ...
        for (i = 0; i < supers.length; i++) {
            num = i + 1;
            $(supers[i])
                .addClass("f-marker f-marker-" + num)
                .data("note", "f-note-" + num);

            supHeight = $(supers[i]).offset().top;
            supAdjustment = $(supers[i])
                .css("top")
                .split("px")[0];

            $(notes[i])
                .addClass("f-note f-note-" + num)
                .css({
                    position: "absolute",
    ...
    Plugin.prototype.positionColumn = function() {
        ...
        if (this.sels.$sups.length === this.sels.$notes.length) {
            ...
            console.log(
                "The number of superscripts don't match the number of notes"
            );
        }
        return isReady;
    };
